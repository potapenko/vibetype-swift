...
| --- | --- | --- | --- | --- |
| Menu bar app shell | Visible menu bar app and Settings entry exist; Start/Stop is still a placeholder path in `vibetype/MenuBarView.swift`. | `references/openwhispr-main/src/helpers/tray.js`; `references/openwhispr-main/src/helpers/menuManager.js` | Run `VT-150` to close out `VT-015`, then use controller slices to replace placeholder recording behavior. | macOS build plus bounded menu-bar runtime QA when visible menu behavior changes. |
| Settings surface | Native Settings window exists with API key, transcription fields, behavior toggles, permission/privacy copy, and Accessibility status; some related tasks remain blocked by stale verification or runtime QA. | `references/openwhispr-main/src/components/SettingsPage.tsx`; `TranscriptionModelPicker.tsx`; `ApiKeyInput.tsx`; `MicrophoneSettings.tsx` | Run `VT-151`, `VT-152`, and `VT-153` to close out `VT-023`, `VT-024`, and `VT-025`; then run `VT-026` after `VT-157` closes `VT-071`. | Unit/build evidence for settings models; Computer Use QA for changed Settings controls when available. |
